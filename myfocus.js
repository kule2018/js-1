    ...
    myFocus.fn.extend({//Method(fn)
        ...
        play: function (funcLastFrame, funcCurrentFrame, seamless) {
            var this_ = this, p = this_.settings, n = this_.HTMLUListLength, t = p.time * 1000, seamless = seamless || false,//是否无缝
                fl = myFocus(this_.HTMLUList).css('float'), isLevel = fl === 'left',//仅支持'left'方向和'top'方向
            ...
            if (p.auto) {//自动切换
                ...
                this_.bind('mouseover', function () {//绑定事件
                    ...
                    if (!this_.isStop) this_.runTimer = setInterval(function () {
                        this_.run('+=1')
                    }, t);
                });
            }
        ...
        bindControl: function ($btnList, params) {//params={thumbShowNum:'略缩图显示数目(如果有)',isRunning:'运行中的标记(当需要判断时)'}
            ...
            if (tsNum) {//thumb
                var fl = $btnList.css('float'), isLevel = fl === 'left' || fl === 'right';
    ...
    myFocus.extend({//Init
        set: function (p, callback) {
            ...
            if (data && data instanceof Array) {
                var myFocus = document.getElementById(id);
                var html = p.isHideLoading ? '' : '<div class="loading"><img src="http://img.gd.sohu.com/img/loading1.gif" alt="请稍候..." /></div>';
                html += '<div class="pic"><ul>';
                for (var i = 0, len = data.length; i < len; i++) {
                    ...
                    var alt = item.alt || '';
                    var text = item.text || '';
                    html += '<li><a href="' + href + '"><img src="' + src + '" thumb="' + thumb + '" alt="' + alt + '" text="' + text + '" /></a></li>'
                }
                html += '</ul></div>';
                myFocus.innerHTML = html;
            }
            /*----added end----*/
        ...
        loadPattern: function (p, callback) {
            ...
            var path = this.getFilePath() + 'mf_pattern/' + p.pattern;
